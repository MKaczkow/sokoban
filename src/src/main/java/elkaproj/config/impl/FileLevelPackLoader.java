...
class FileLevelPackLoader implements ILevelPackLoader {
    ...
    public ILevelPack loadPack(String id) throws IOException {
        ...
        XmlLevelPackMeta xlpm = null;
        try {
            JAXBContext jaxbctx = JAXBContext.newInstance(XmlLevelPackMeta.class);
            Unmarshaller jaxb = jaxbctx.createUnmarshaller();
            xlpm = (XmlLevelPackMeta) jaxb.unmarshal(meta);
        ...
        try {
            JAXBContext jaxbctx = JAXBContext.newInstance(XmlLevelMeta.class);
            Unmarshaller jaxb = jaxbctx.createUnmarshaller();
            for (File leveldef : leveldefs) {
                levels.add(this.loadLevel((XmlLevelMeta) jaxb.unmarshal(leveldef), pack));
            }
        } catch (JAXBException e) {
            throw new IOException(e);
        }

        return new XmlLevelPack(xlpm, levels);
    }

    private ILevel loadLevel(XmlLevelMeta xdef, File pack) throws IOException {
        ...
        return new XmlLevel(xdef, new Dimensions(width, height), tiles);
    }
    ...
    private static class XmlLevel implements ILevel {
        ...
        public XmlLevel(XmlLevelMeta xdef, Dimensions dims, LevelTile[][] tiles) {
    ...
    private static class XmlLevelPack implements ILevelPack {

        private final String name, id;
        private final List<ILevel> levels;

        public XmlLevelPack(XmlLevelPackMeta xdef, List<ILevel> levels) {
            this.name = xdef.name;
            this.id = xdef.id;
            this.levels = levels;
        }
